...
public class DynamicListView extends ListView {
    ...
    private final int SMOOTH_SCROLL_AMOUNT_AT_EDGE = 135;
            ...
            switch (event.getAction() & MotionEvent.ACTION_MASK) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (mCellIsMobile) {
                        ...
                        if (absX > 10 && !enableX && !enableY) {
                            enableX = true;
                            enableY = false;
                            enableStartY = y;
                        } else if (absY > 10 && !enableY && !enableX) {
                            enableX = false;
                            enableY = true;
                            enableStartX = x;
                        }
                        ...
                        if (enableX && Math.abs(deltaX) > rowWidth / 2) {
                            handleCellDelete();
...
//                            touchEventsEnded();
                        ...
                        if (enableY) {
                            ...
                            if (deltaX > (rowWidth / 2) && !isTrasboxHover) {
                                hovoerTrashbox(true);
                            } else if (deltaX < (rowWidth / 2) && isTrasboxHover) {
                                hovoerTrashbox(false);
                            }
                        }
...
//        }
        ...
        listener.deleteElement(originalItem);
        ((BaseAdapter) getAdapter()).notifyDataSetChanged();
    }
    ...
    public interface Listener {
        ...
        void deleteElement(int originalItem);
    }
}
